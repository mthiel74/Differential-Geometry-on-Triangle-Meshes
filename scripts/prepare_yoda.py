#!/usr/bin/env python3
"""prepare_yoda.py — one-off preprocessing of a high-resolution 3D scan into
clean, decimated triangle meshes usable by the pure-Wolfram DDG pipeline.

The source is the 503k-vertex / 1M-triangle scanned "Yoda" OBJ from the
Wolfram3DObjects repository (texture seams split it into ~91k apparent
components). We weld duplicated vertices, keep the largest connected
component, decimate with quadric error metrics, orient consistently, and
normalise to a unit bounding sphere centred at the origin.

Outputs (committed to data/ so the WL pipeline never needs Python):
  data/yoda.obj         ~12000 faces  (curvature, geodesics)
  data/yoda_coarse.obj  ~2400 faces   (dense spectral solve)

Usage:  python3 scripts/prepare_yoda.py [SRC_OBJ]
...
SRC = sys.argv[1] if len(sys.argv) > 1 else os.path.expanduser(
    "~/GitHub/Wolfram3DObjects/yoda_mesh_obj/mesh.obj")
...
def main():
    ...
    # weld texture-seam-split vertices, then clean and keep the main body
    ...
    for n_faces, name in [(12000, "yoda.obj"), (2400, "yoda_coarse.obj")]:
