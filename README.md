...
emergent space. A **rule search** (`scripts/search_wolframmodel.wls`, scaled to **160 rules** in
`scripts/enumerate_wolframmodel.wls` with an exponential-vs-power-law fit) finds
a stark "missing middle" — **103 of 160 grow exponentially, none ~3D, only 2 in
the flat 2D/3D band** — quantifying why physically realistic rules are rare.
**Forman–Ricci** (`wolfram/forman.wls`) is added as a ~100× faster, degree-based
complement to Ollivier–Ricci (per-edge correlation r≈0.6–0.9, with a degree
offset). Being cheap, it can *drive* a **curvature-guided coarsening**
(`wolfram/coarsen.wls`, `GraphDDG`ricciCoarsen`): contracting max-Forman edges
renormalizes a 2D mesh-graph while **preserving its dimension** (≈1.8 — scale
invariance) and **recovers planted communities** (~82% purity).
`wolfram/graphgeometry.wl` holds the graph operators.
